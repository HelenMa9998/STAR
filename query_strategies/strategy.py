...
class Strategy:
    ...
    def train(self, rd, training_method, handler):
        labeled_idxs, labeled_data = self.dataset.get_labeled_data(handler)
        print(len(labeled_idxs))
        val_data = self.dataset.get_val_data(handler)
    ...
    def predict(self, data, num_slices_per_patient):  # 在test data上predict
        preds = self.net.predict(data, num_slices_per_patient)
        return preds
